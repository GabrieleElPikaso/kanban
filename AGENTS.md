...
- On Windows, npm-installed agent CLIs like `claude` and `codex` may resolve to `.cmd` shims, especially when Kanban is started from Git Bash/MSYS-style shells. Do not blindly wrap those launches in `cmd.exe`: large sidebar prompts can exceed the Windows `cmd.exe` command-line limit and fail with `The command line is too long.` Prefer resolving the shim to its underlying `.exe` or Node entrypoint and launching that directly.
...
- Board column moves are a whitelist, not a blacklist: every allowed cross-column transition lives in `web-ui/src/state/drag-rules.ts` and must define its agent-lifecycle side effects (kickoff, stop, cleanup, resume) in `handleDragEnd` in `web-ui/src/hooks/use-board-interactions.ts`. When adding a backward transition (e.g. review -> backlog), account for the session-sync effect in that same hook: `stopTaskSession` marks the session `interrupted`, and the effect auto-moves cards with newly-interrupted sessions to trash unless their column is explicitly excluded (`trash` and `backlog` are). Leaving an active column (trash or backlog) deletes the task worktree; a later start recreates it fresh from `baseRef`.
- Provider ids that cross the SDK boundary (settings reads/saves, launch-config resolution, model listing) must route through `normalizeSdkProviderId` in `src/cline-sdk/sdk-provider-boundary.ts`: other Cline surfaces can write legacy aliases ("openai" -> "openai-native", "togetherai" -> "together", "sap-ai-core" -> "sapaicore") into `~/.cline` provider settings, and the llms registry only knows canonical ids — a legacy id passed verbatim throws `Unknown or disabled provider "openai"` even when registration is fine. `migrateSdkLegacyProviderIds()` rewrites persisted settings stored under legacy keys (canonical entry wins on conflict) on the same memoized provider-load path as custom-provider registration.
